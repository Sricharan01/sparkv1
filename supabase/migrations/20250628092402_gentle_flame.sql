/*
  ...
  1. New Tables
    - `documents` - Store processed documents with OCR data and metadata
  ...
  3. Performance
    - Add comprehensive indexes for fast queries
    - Include GIN indexes for JSONB and array columns
    - Add text search indexes for OCR content

  4. Data Integrity
    - Add check constraints for status values
    - Create foreign key relationships where appropriate
...
-- Create policies for audit logs (matching the provided schema)
...
-- Create policies for users (matching the provided schema)
