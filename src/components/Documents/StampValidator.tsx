...
import { officialStampService, DocumentValidationResult, StampDetectionResult, SignatureDetectionResult } from '../../services/officialStampService';
import { useAuth } from '../../contexts/AuthContext';

interface StampValidatorProps {
  documentImage: string | File;
  onValidationComplete: (result: DocumentValidationResult) => void;
  className?: string;
}

export function StampValidator({ 
  documentImage, 
  ...
  const handleAnalyze = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const result = await officialStampService.analyzeDocument(documentImage, user.id);
  ...
  const downloadStampImage = (stamp: StampDetectionResult) => {
  ...
  const downloadSignatureImage = (signature: SignatureDetectionResult) => {
    const link = document.createElement('a');
  ...
  const getMasterListMatchIcon = (matches: boolean) => {
    return matches ? (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900">Official Stamp & Signature Validation</h3>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                <div className="flex items-center">
                  {getMasterListMatchIcon(validationResult.stamps.matchesMasterList)}
                  <span className="ml-2 text-sm font-medium">
                    Official Stamp Match: {validationResult.stamps.matchesMasterList ? 'Y' : 'N'}
                  </span>
                </div>
              </div>
        ...
        <div className="mt-4 bg-blue-50 rounded-lg p-4">
          <h4 className="text-sm font-medium text-blue-900 mb-2">Official Stamp Master List:</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {officialStamps.map((stamp, index) => (
              <div key={stamp.id} className="text-xs text-blue-800 flex items-center">
                <span className="inline-block w-4 text-center">{index + 1}.</span>
                <span className="ml-1">{stamp.name}</span>
              </div>
            ))}
          </div>
        </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className={`px-2 py-1 ${stamp.matchesMasterList ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'} text-xs rounded-full`}>
                            {stamp.type.replace('_', ' ')}
                          </span>
                          <span className={`px-2 py-1 ${stamp.matchesMasterList ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'} text-xs rounded-full`}>
